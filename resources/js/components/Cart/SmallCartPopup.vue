<template>
    <div v-show="cart?.count > 0" class="absolute top-0 right-0 w-96 z-[1000]">
        <div class="h-8 bg-transparent"></div>
        <div
            class="bg-white rounded-lg border divide-y overflow-y-auto max-h-[32rem]"
        >
            <div class="px-8 py-2 flex text-gray-600 text-sm">
                <div>
                    Total: <span class="font-semibold">{{ amount }}</span>
                </div>
                <div class="ml-auto">
                    Items: <span class="font-semibold">{{ cart.count }}</span>
                </div>
            </div>
            <template
...
export default defineComponent({
    ...
    methods: {
        ...
        async initItems(): void {
            this.cart = await CartService.cart();
        },
        updateCart(cart) {
            this.cart = cart;
        },
