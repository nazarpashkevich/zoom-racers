<template>
  <Link
    class="flex flex-col gap-2 hover:bg-gray-50 py-4 px-6 cursor-pointer"
    :href="route('events.show', event.id)"
  >
    <div
      class="bg-cover bg-center h-64 rounded-md"
      :style="{ backgroundImage: `url('${event.picture}')` }"
    ></div>
    <div class="text-xl font-semibold text-left hover:text-indigo-900 my-auto">
      {{ event.title }}
    </div>
